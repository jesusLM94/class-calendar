...
const ClassBlock: React.FC<ClassBlockProps> = ({ assignment, isDragging = false }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  const getClassTypeStyles = (classType: string): string => {
    switch (classType) {
      case 'power':
  ...
  const getClassTypeIcon = (classType: string): string => {
    switch (classType) {
      case 'power':
        return '💪';
      case 'cycling':
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className={blockClasses}>
      <div className="flex items-center space-x-2 mb-1">
        <span className="text-lg">{getClassTypeIcon(assignment.classType)}</span>
        <span className="font-semibold text-sm uppercase tracking-wide">
          {assignment.classType}
        </span>
      </div>

      <div className="font-medium text-gray-800 mb-1">{assignment.coachName}</div>
